...
const FeedbackForm = ({ destination = "Ames" }) => {
    ...
    const fetchFeedback = async () => {
      try {
        const res = await axios.get(
          `http://localhost:8080/api/feedback/by-city/${destination}` // Updated port
        );
  ...
  const handleSubmit = async (e) => {
    ...
    const newEntry = {
      rating,
      message: comment, // ✅ must match backend field
      user: "rishabh@iastate.edu", // 🔧 replace with dynamic user if needed
      city: destination, // ✅ used for /by-city route
      destination,
    };

    try {
      await axios.post("http://localhost:8080/api/feedback", newEntry); // Updated port
      const res = await axios.get(
        `http://localhost:8080/api/feedback/by-city/${destination}` // Updated port
      );
      setFeedback(res.data);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-400 via-indigo-200 to-pink-100 py-0 px-2 flex flex-col items-center">
      ...
      <div className="w-full max-w-2xl mx-auto text-center py-12">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-lg text-gray-600 max-w-xl">
            Your feedback helps other travelers and makes our community better.
      ...
      <div className="w-full max-w-2xl mb-20">
        ...
        {feedback.length === 0 ? (
          <p className="text-gray-600 bg-white/70 rounded-xl p-6 text-center shadow">
